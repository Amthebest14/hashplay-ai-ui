...
/**
 * @title HashplayArenaV4
 * @notice Casino-grade wagering contract for Hedera using Pseudo-Randomness.
 *         Features:
 *         - 30% Win Rate: House-favored odds on all game types
 *         - Secure Randomness: Powered by block hash, timestamp, and sender (Pseudo-random)
 *         - Airdrop Points: Automated on-chain XP mapping (No associations needed)
 *         - Treasury: Automated 5% fee routing on user losses
 */
contract HashplayArenaV4 is Ownable, ReentrancyGuard {
    ...
    function play(uint8 gameType, uint8 prediction) external payable nonReentrant {
        ...
        if (gameType == 1) {
            // DICE GAME — Visual dice + 30% win rate
            // Generate dice values for display
            uint256 die1 = (randValue % 6) + 1;
            uint256 die2 = ((randValue / 10) % 6) + 1;
            rollResult = die1 + die2;

            // 30% win check using separate bits of the random seed
            uint256 winRoll = ((randValue >> 128) % 100) + 1;
            if (winRoll <= 30) {
                won = true;
            }
            ...
            // COIN FLIP — 30% win rate
            // Roll 1-100: Heads wins 1-30, Tails wins 71-100, House wins 31-70
            rollResult = (randValue % 100) + 1;

            if (prediction == 1 && rollResult <= 30) {
                won = true;  // Heads: 30%
            } else if (prediction == 2 && rollResult >= 71) {
                won = true;  // Tails: 30%
            }
            // Results 31-70: House always wins (40% house edge zone)
