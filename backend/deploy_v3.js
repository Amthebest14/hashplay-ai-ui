...
async function main() {
    ...
    const artifactPath = path.resolve(__dirname, 'artifacts', 'contracts', 'HashplayArenaV3.sol', 'HashplayArenaV3.json');
    const { abi, bytecode } = JSON.parse(fs.readFileSync(artifactPath, 'utf8'));

    // Deploy contract
    console.log("Rolling out HashplayArenaV3...");
    const factory = new ethers.ContractFactory(abi, bytecode, wallet);

    // Adjusted gas limit for Mainnet deployments to fit small HBAR balances (1.5M)
    ...
    console.log(`\n✅ HashplayArenaV3 deployed to ${network}: ${deployedAddress}`);
