...
async function main() {
    ...
    console.log(`--- DEPLOYING TO HEDERA ${network.toUpperCase()} ---`);
    ...
    const artifactPath = path.resolve(__dirname, 'artifacts', 'contracts', 'HashplayArenaV2.sol', 'HashplayArenaV2.json');
    const { abi, bytecode } = JSON.parse(fs.readFileSync(artifactPath, 'utf8'));

    // Deploy contract
    console.log("Rolling out HashplayArenaV2...");
    const factory = new ethers.ContractFactory(abi, bytecode, wallet);

    // High gas limit for Mainnet deployments (5M)
    const contract = await factory.deploy(treasuryEvmAddress, { gasLimit: 5000000 });

    console.log(`Waiting for deployment transaction ${contract.deploymentTransaction().hash}...`);
    await contract.waitForDeployment();

    const deployedAddress = await contract.getAddress();
    console.log(`\n✅ HashplayArenaV2 deployed to ${network}: ${deployedAddress}`);
